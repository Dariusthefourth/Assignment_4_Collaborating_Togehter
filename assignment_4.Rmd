...
The variable rgdp60 in the GrowthSW dataset is a measure of real GDP per capita in 1960, converted to 1960 US dollars. It provides information about the economic output per person adjusted for inflation, allowing for comparisons across countries. By including rgdp60, researchers can examine the relationship between the initial level of real GDP per capita and the subsequent economic growth rates of different countries. This variable helps to assess the influence of initial economic conditions on long-term economic development.
...
model2 <- update(model1, . ~ . + control_variable1)
```
